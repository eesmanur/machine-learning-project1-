...
def evaluate_metrics_for_class(X, y, y_class):
    ...
    for train_index, test_index in kf.split(X):
        ...
        cluster_labels_train = clustering.fit_predict(X_train_class)
        cluster_labels_test = clustering.fit_predict(X_test_class)

        silhouette_train = silhouette_score(X_train_class, cluster_labels_train)
        silhouette_test = silhouette_score(X_test_class, cluster_labels_test)
        calinski_harabasz_train = calinski_harabasz_score(X_train_class, cluster_labels_train)
        calinski_harabasz_test = calinski_harabasz_score(X_test_class, cluster_labels_test)
        davies_bouldin_train = davies_bouldin_score(X_train_class, cluster_labels_train)
        davies_bouldin_test = davies_bouldin_score(X_test_class, cluster_labels_test)
        ari_train = adjusted_rand_score(y_train[y_train == y_class], cluster_labels_train)
        ari_test = adjusted_rand_score(y_test[y_test == y_class], cluster_labels_test)
        mi_train = mutual_info_score(y_train[y_train == y_class], cluster_labels_train)
        mi_test = mutual_info_score(y_test[y_test == y_class], cluster_labels_test)

        silhouette_scores.append(silhouette_test)
        calinski_harabasz_scores.append(calinski_harabasz_test)
        davies_bouldin_scores.append(davies_bouldin_test)
        ari_scores.append(ari_test)
        mi_scores.append(mi_test)
