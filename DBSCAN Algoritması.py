...
def evaluate_metrics_for_class(X, y, y_class):
    ...
    for train_index, test_index in kf.split(X):
        ...
        dbscan.fit(X_train_class)

        cluster_labels = dbscan.fit_predict(X_test_class)

        if len(np.unique(cluster_labels)) > 1:
            silhouette = silhouette_score(X_test_class, cluster_labels)
            calinski_harabasz = calinski_harabasz_score(X_test_class, cluster_labels)
            davies_bouldin = davies_bouldin_score(X_test_class, cluster_labels)
            ari = adjusted_rand_score(y_test[y_test == y_class], cluster_labels)
            mi = mutual_info_score(y_test[y_test == y_class], cluster_labels)
